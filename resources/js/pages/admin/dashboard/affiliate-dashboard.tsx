...
export default function AffiliateDashboard({ user, stats, filters }: AffiliateStatsProps) {
  ...
  const handleCopyAffiliateLink = async () => {
    try {
      const link = `https://arsacendekia.id/register?ref=${user.affiliate_code}`;
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Filter */}
      <div className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-indigo-50/70 via-white to-sky-50/60 p-5 dark:from-indigo-950/40 dark:via-background dark:to-sky-900/20">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -left-6 top-4 h-24 w-24 rounded-full bg-indigo-400/10 blur-xl dark:bg-indigo-500/15" />
          <div className="absolute bottom-2 right-4 h-28 w-28 rounded-full bg-sky-400/10 blur-xl dark:bg-sky-500/15" />
        </div>
      ...
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
        {topStatsCards.map((card, i) => (
          <div
            key={i}
            className="group relative overflow-hidden rounded-xl border bg-white/60 p-5 backdrop-blur
                       dark:bg-white/5"
          >
            <div
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        {/* Recent Referrals */}
        <div className="relative overflow-hidden rounded-xl border bg-white p-6 dark:from-indigo-950/40 dark:via-background dark:to-sky-900/20 lg:col-span-2">
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute -left-8 top-6 h-36 w-36 rounded-full bg-indigo-400/15 blur-2xl dark:bg-indigo-500/25" />
            <div className="absolute bottom-0 right-0 h-48 w-48 rounded-full bg-sky-400/15 blur-2xl dark:bg-sky-600/25" />
          </div>
            <div className="relative mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold">Referral Terbaru</h3>
                <p className="text-sm text-muted-foreground">
                  Pendaftaran terbaru melalui link Anda.
                </p>
              </div>
            ...
            <div className="relative space-y-4">
              {stats.recent_referrals.length === 0 ? (
                ...
                stats.recent_referrals.map((ref) => (
                  <div
                    key={ref.id}
                    className="relative flex items-center gap-4 rounded-lg border bg-white/70 p-4 backdrop-blur transition
                               hover:shadow-sm dark:bg-white/10"
                  >
                    ...
                    <div className="flex-1">
                      <p className="font-medium leading-tight">{ref.invoice.user.name}</p>
                      <p className="text-xs text-muted-foreground">{getInvoiceItemName(ref.invoice)}</p>
                    </div>
                    <div className="text-right">
                      <div className="font-semibold text-green-600">{formatCurrency(ref.amount)}</div>
                      <div className="text-[11px] text-muted-foreground">Komisi {user.commission}%</div>
                    </div>
                  </div>
                ))
              )}
        ...
        <div className="space-y-6">
          <div className="relative overflow-hidden rounded-xl border bg-white p-6 dark:from-indigo-950/40 dark:via-background dark:to-sky-900/20">
            <div className="pointer-events-none absolute inset-0">
              <div className="absolute -left-6 -top-4 h-24 w-24 rounded-full bg-indigo-400/15 blur-2xl dark:bg-indigo-500/25" />
              <div className="absolute bottom-0 right-0 h-32 w-32 rounded-full bg-sky-400/15 blur-2xl dark:bg-sky-600/25" />
            </div>
            <div className="relative">
              <h3 className="mb-2 text-lg font-semibold">Link Afiliasi</h3>
              <p className="mb-4 text-sm text-muted-foreground">Bagikan untuk mendapatkan komisi.</p>
              <div className="flex items-center gap-2 rounded-lg border bg-white/70 p-3 backdrop-blur dark:bg-white/10">
                <p className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-muted-foreground">
                  {`arsacendekia.id/register?ref=${user.affiliate_code}`}
                </p>
                <button
          ...
          <div className="relative overflow-hidden rounded-xl border bg-white p-6 dark:from-indigo-950/40 dark:via-background dark:to-sky-900/20">
            <div className="pointer-events-none absolute inset-0">
              <div className="absolute left-4 top-4 h-20 w-20 rounded-full bg-indigo-400/15 blur-xl dark:bg-indigo-500/25" />
            </div>
            <div className="relative">

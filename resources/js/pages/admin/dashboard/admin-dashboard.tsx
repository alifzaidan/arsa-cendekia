...
export default function AdminDashboard({ stats, filters }: StatsProps) {
  ...
  const [endDate, setEndDate] = useState<Date | undefined>(() =>
    filters?.end_date ? new Date(filters.end_date + 'T00:00:00') : undefined,
  );
  ...
  const bottomStatsCards = [
    {
      ...
      icon: <UserPlus className="size-5 text-purple-600 dark:text-purple-300" />,
      subValue: `+${stats.new_users_last_week.toLocaleString('id-ID')} (7 hari)`,
      gradient: 'from-purple-500/15 via-fuchsia-400/10 to-pink-400/10',
    },
    {
      title: 'Total Mentor',
      value: stats.total_mentors.toLocaleString('id-ID'),
      icon: <UserCheck className="size-5 text-green-600 dark:text-green-300" />,
      subValue: `+${stats.new_mentors_last_week.toLocaleString('id-ID')} (7 hari)`,
    ...
    },
    {
      ...
      icon: <Network className="size-5 text-sky-600 dark:text-sky-300" />,
      subValue: `+${stats.new_affiliates_last_week.toLocaleString('id-ID')} (7 hari)`,
    ...
    },
    {
      ...
      icon: <BookOpen className="size-5 text-indigo-600 dark:text-indigo-300" />,
      gradient: 'from-indigo-500/15 via-indigo-400/10 to-sky-400/10',
    },
    {
      title: 'Total Webinar',
      value: stats.total_webinars.toLocaleString('id-ID'),
      icon: <Mic className="size-5 text-pink-600 dark:text-pink-300" />,
      gradient: 'from-pink-500/20 via-rose-400/10 to-fuchsia-400/10',
    },
  ];
  ...
  return (
    <div className="space-y-8">
      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl border bg-gradient-to-br from-indigo-50 via-white to-sky-50 p-6 dark:from-indigo-950/50 dark:via-background dark:to-sky-900/25">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -left-10 -top-12 h-44 w-44 rounded-full bg-indigo-400/20 blur-2xl dark:bg-indigo-600/25" />
          <div className="absolute bottom-0 right-0 h-60 w-60 rounded-full bg-sky-400/20 blur-3xl dark:bg-sky-700/25" />
          <div className="absolute left-1/3 top-1/2 h-48 w-48 -translate-y-1/2 rounded-full bg-indigo-300/10 blur-3xl dark:bg-indigo-400/10" />
        </div>
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Ringkasan performa platform dan aktivitas terbaru.
            </p>
          </div>
      ...
      {/* Filter */}
      <div className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-indigo-50/70 via-white to-sky-50/60 p-5 dark:from-indigo-950/40 dark:via-background dark:to-sky-900/25">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -left-6 top-4 h-24 w-24 rounded-full bg-indigo-400/15 blur-xl dark:bg-indigo-600/25" />
          <div className="absolute bottom-2 right-4 h-28 w-28 rounded-full bg-sky-400/15 blur-xl dark:bg-sky-600/25" />
        </div>
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {topStatsCards.map((card, i) => (
          <div
            key={i}
            className="group relative overflow-hidden rounded-xl border bg-white/60 p-5 backdrop-blur dark:bg-white/5"
          >
            ...
            <div className="relative mt-3 space-y-2">
              ...
              {card.percentage !== undefined && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">{card.comparison}</span>
                </div>
              )}
            </div>
            <div className="pointer-events-none absolute -right-6 -top-6 h-20 w-20 rounded-full bg-white/40 blur-xl dark:bg-white/10" />
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <div className="space-y-6">
          <div className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-indigo-50 via-white to-sky-50 p-6 dark:from-indigo-950/40 dark:via-background dark:to-sky-900/25">
            <div className="pointer-events-none absolute inset-0">
              <div className="absolute -left-6 -top-4 h-28 w-28 rounded-full bg-indigo-400/15 blur-2xl dark:bg-indigo-600/25" />
              <div className="absolute bottom-0 right-0 h-32 w-32 rounded-full bg-sky-400/15 blur-2xl dark:bg-sky-700/25" />
            </div>
            <div className="relative">
              ...
              <div className="mt-4 space-y-5">
                {stats.recent_sales.length === 0 ? (
                  <div className="flex flex-col items-center justify-center gap-4 rounded-lg border border-dashed bg-white/70 py-10 text-sm text-muted-foreground backdrop-blur dark:bg-white/5">
                  ...
                  stats.recent_sales.slice(0, 6).map((sale) => (
                    <div key={sale.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 leading-tight">
                        ...
                        <p className="text-[11px] text-muted-foreground">{getInvoiceItemName(sale)}</p>
                      </div>
          ...
          <div className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-indigo-50 via-white to-sky-50 p-6 dark:from-indigo-950/40 dark:via-background dark:to-sky-900/25">
            <div className="pointer-events-none absolute inset-0">
              <div className="absolute left-4 top-4 h-24 w-24 rounded-full bg-indigo-400/15 blur-xl dark:bg-indigo-600/25" />
              <div className="absolute bottom-2 right-4 h-28 w-28 rounded-full bg-sky-400/15 blur-xl dark:bg-sky-600/25" />
            </div>
            <div className="relative">
              <h3 className="mb-2 text-lg font-semibold">Produk Terpopuler</h3>
              <p className="mb-4 text-xs text-muted-foreground">Berdasarkan jumlah pendaftar.</p>
                    ...
                    <div key={p.id} className="flex items-center justify-between">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          ...
                          <span className="text-[11px] text-muted-foreground">{formatCurrency(p.price)}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-sm font-semibold">{p.enrollment_count}</span>
                        <p className="text-[10px] text-muted-foreground">pendaftar</p>
                      </div>
                    </div>
                  ))
                )}
          ...
          {/* Bottom Stats Mini */}
          <div className="space-y-4">
            <h4 className="text-sm font-semibold tracking-tight">Ringkasan Lainnya</h4>
            <div className="grid gap-4 sm:grid-cols-2">
              {bottomStatsCards.map((card, i) => (
                <div
                  key={i}
                  className="group relative overflow-hidden rounded-lg border bg-white/60 p-4 backdrop-blur transition hover:shadow-sm dark:bg-white/5"
                >
                  <div className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${card.gradient} opacity-90`} />
                  <div className="relative flex items-start justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-lg font-bold leading-none">{card.value}</p>
                      {card.subValue && (
                        <p className="mt-1 text-[11px] text-muted-foreground">{card.subValue}</p>
                      )}
                    </div>
                    <div className="flex h-9 w-9 items-center justify-center rounded-full bg-white/70 ring-1 ring-black/5 dark:bg-white/10">
                      {card.icon}
                    </div>
                  </div>
                </div>

...
const formSchema = z.object({
    ...
    program_type: z.enum(['course', 'webinar']).refine((val) => val !== undefined, {
...
export default function CreateCertificate({ designs, signs, courses, webinars, prefilledData = {} }: CertificateCreateProps) {
    ...
    const isPrefilledProgram = isPrefilledFromCourse || isPrefilledFromWebinar;
    ...
    const getSelectedProgramName = () => {
        ...
        const selectedId = form.watch(programFieldName as 'course_id' | 'webinar_id');
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 items-start gap-6 lg:grid-cols-2">
                        ...
                        <div className="space-y-6 rounded-lg border p-4">
                            ...
                            {programType && (
                                <FormField
                                    control={form.control}
                                    name={getProgramFieldName() as 'course_id' | 'webinar_id'}
                                    render={({ field }) => (
                                        <FormItem className="flex flex-col">
                                            <FormLabel>
                                                {programType === 'course' ? 'Kelas Online' : 'Webinar'} <span className="text-red-500">*</span>
                                            </FormLabel>
                                            {isPrefilledProgram ? (
                                                                ...
                                                                <CommandGroup>
                                                                    {getProgramOptions().map((option) => (
                                                                        <CommandItem
                                                                            ...
                                                                            onSelect={() => {
                                                                                form.setValue(
                                                                                    getProgramFieldName() as 'course_id' | 'webinar_id',
                                                                                    option.id,
                                                                                );
